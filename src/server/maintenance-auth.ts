...
export const requireMaintenanceInRoute = async (request: NextRequest) => {
  const configuredToken = process.env.MAINTENANCE_WORKER_TOKEN?.trim();
  if (configuredToken && configuredToken.length < MINIMUM_TOKEN_LENGTH) {
    return {
      ...
      response: NextResponse.json(
        {
          error: {
            code: "MAINTENANCE_AUTH_MISCONFIGURED",
            message: `MAINTENANCE_WORKER_TOKEN must contain at least ${MINIMUM_TOKEN_LENGTH} characters.`,
          },
        },
        { status: 503 }
      ),
  ...
  if (
    ...
    return {
      principal: { id: "maintenance-worker", kind: "service" as const },
      response: null,
    };
  }
