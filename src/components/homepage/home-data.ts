...
// ---------------------------------------------------------------------------
// Map routes: where the open verified opportunities take place.
...
export const ROUTE_ORIGIN: MapPoint = { lat: -15.79, lon: -47.88 };

const OPPORTUNITY_LOCATIONS: Record<string, MapPoint> = {
  "lester-b-pearson-2027": { lat: 43.66, lon: -79.4 },
  "ubc-international-scholars-2027": { lat: 49.26, lon: -123.25 },
  "yale-young-global-scholars-2027": { lat: 41.31, lon: -72.93 },
  "febrace-2027": { lat: -23.56, lon: -46.73 },
  "one-2026": { lat: -15.79, lon: -47.88 },
};

export interface MapDestination extends MapPoint {
  country: string;
  id: string;
  scope: OpportunityScope;
}

export const getMapDestinations = (
  featured: FeaturedOpportunity[]
): MapDestination[] =>
  featured.flatMap((opportunity) => {
    const location = OPPORTUNITY_LOCATIONS[opportunity.id];
    return location
      ? [
          {
            ...location,
            country: opportunity.place,
            id: opportunity.id,
